...
class Suggestion extends CI_Controller {
	...
	public function _remap($title, $name) {
		...
		$data['menu_title'] = "suggestion";
		$view_name = '/suggestion/' . $title;
	...
	private function view_suggestions($view_name,$data){
		if($data['req_id']!=NULL){
	...
	private function suggest_to_tutor($view_name,$data){
		$this -> load -> view($view_name, $data);
	}
	
	private function suggest_to_admin($view_name,$data){
		$this -> load -> view($view_name, $data);
	}
}